...
my $i = 0;
my @files = <$DIR/$SUBDIR/work.err-cor/binmodel.err-cor/moses.mert.?.ini>;
@files = map { s/\.\d\.\d.ini/.ini/; $_ } @files;
if ($ITERATION > 0) {
    @files = <$DIR/$SUBDIR/work.err-cor/binmodel.err-cor/moses.mert.$ITERATION.ini>;
    @files = map { s/\.\d\.$ITERATION.ini/.$ITERATION.ini/; $_ } @files;
}
print $ITERATION, " ", scalar @files, "\n";
...
open(INIS, "paste $DIR/$SUBDIR/work.err-cor/binmodel.err-cor/moses.mert.$suffix.ini |") or die "Problem 1\n";
my @inis = <INIS>;
close(INIS);

my @names = qw(UnknownWordPenalty WordPenalty PhrasePenalty TranslationModel LexicalReordering Distortion LM OpSequenceModel EditSequenceModel EditOps VW BNLM NLM NMT CharOps LevDis);
