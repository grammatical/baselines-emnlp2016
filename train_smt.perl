...
die "Specify a path to SRILM.\n" if ( not $SRILMDIR or not -e $SRILMDIR );
...
if ( $LEVENSHTEIN and $EDITOPS ) {
    die "Options --levenshtein and --editops are not prepared to run together.\n";
}
...
if ( not all_exist(@PT_TEST) ) {
    ...
    my @STEPS = (
        ...
        [ "3", ["$MODEL_DIR/aligned.grow-diag-final"] ],
        [   "4",
            [ "$MODEL_DIR/lex.$TFACTOR.e2f", "$MODEL_DIR/lex.$TFACTOR.f2e" ]
        ],
        [   "5",
...
if ( not -e "$BINMODEL_DIR/$INI" ) {
    ...
    while (<INI1>) {
        ...
        if (/\[feature\]/) {
            ...
            if ($OSM) {
                print INI2
                    "OpSequenceModel path=$BINMODEL_DIR/osm.kenlm input-factor=0 output-factor=0 support-features=no num-features=1\n";
            }
            if ($ESM) {
                print INI2
                    "EditSequenceModel path=$BINMODEL_DIR/esm.kenlm input-factor=0 output-factor=0 num-features=1\n";
            }
            if ($LEVENSHTEIN) {
                print INI2 "EditOps scores=l\n";
            }
            if ($EDITOPS) {
            ...
            if ($WCLM) {
                print INI2
                    "Generation name=Generation0 num-features=0 input-factor=0 output-factor=1 path=$WC\n";
            }
            next;
        }
